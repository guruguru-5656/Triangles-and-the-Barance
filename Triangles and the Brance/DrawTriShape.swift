...
struct DrawTriShape:Shape{
    ...
    init(in coordinate:[TriCoordinate],scale:CGFloat ,offset: CGFloat){
    ...
    func getVertexPoint(coordinates:[TriCoordinate]) -> [CGPoint]{
        let vertex = coordinates.map{
            CGPoint(x:$0.getDrowPoint().x * self.scale ,y:$0.getDrowPoint().y * self.scale)
        }
        return vertex
    }
    ///中心点を設定する
    func getCenterPoint(vertex:[CGPoint]) -> CGPoint{
    ...
    func makeTransform(center:CGPoint,scale:CGFloat) -> CGAffineTransform?{
